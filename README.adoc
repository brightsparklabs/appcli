...
Appcli supports the ability to run archiving on the data directory for the purposes of log rotation.
These are defined as rulesets in the `stack-setting.yml` file which is then called through the `archive` command.
A ruleset consists of 1 or many rules to be executed.
Currently the following ruletypes are supported:

* `Compress` => Create a new `.tgz` archive file and push included files into it.
...
The `backup` command optionally takes an argument corresponding to the `name` of the archive ruleset to run.
