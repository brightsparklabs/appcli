...
Appcli supports the ability to run archiving on files within the `data` directory for the purposes of log rotation.
These are defined as rulesets in the `stack-setting.yml` file which is then called through the `archive` command.
For example:
...
A ruleset consists of one or more rules to be executed.
The rule `type` can be any of the following:

* `Compress` => Create a new `.tgz` archive file containing all files matched by the `include_list`.
* `Purge` => Remove included files from the data directory.

==== Configuration Options

There are a number of fields which can be configured for each rule:

* `include_list` => List of glob patterns to use when searching for file matches.
  NOTE: This does not match any hidden files.
* `archive_file` => (Compress only) The name of the archive file to created.
  NOTE: This supports datetime through the use of the `%<value>` notation.
  See https://devhints.io/datetime[Datetime Format String]

Leaving the `name` out of the command results in all rulesets being executed.
NOTE: Whilst the execution order of the rules in a set is guaranteed, the execution order of the rulesets themselves is not.
...
- name: 'logs'
  ...
  - name: 'archive-logs'
    type: 'compress'
    include_list: ['service1/**.log']
    archive_file: 'service1/logs-%Y%m.tgz'
  - name: 'prune-logs'
    type: 'prune'
    include_list ['service1/**.log']
----

This exposes 2 rulesets:

* `cache` => Clears all the lock files for `service1` and temp files for `service2`.
* `logs` => Push all the log files for `service1` into a dated archive, and then remove them from disk.

These can then be integrated into something like a cron job.
...
0 0 * * * NO_TTY=true ./myapp archive cache

# Rotate the logs every month.
0 * 1 * * NO_TTY=true ./myapp archive logs
----
...
--
[horizontal]
archive::: Perform archiving of the data directory.
...
Perform archiving to `.tgz` files and file/directory purging of the data directory for the purposes of things like log rotation.
Archiving rulesets are configured in the `stack-setting.yml` file and callable through the cli.
An archiving ruleset can consist of 1 or many individual rules.

Usage: `./myapp archive [OPTIONS] [ARGS]`

Options::
-–pre-stop-services/-–no-pre-stop-services::: Whether to stop services before performing archive.
-–post-start-services/-–no-post-start-services::: Whether to start services after performing archive.
-–help::: Show the help message and exit.

The `archive` command optionally takes an argument corresponding to the `name` of the archive ruleset to run.
If no `name` is provided, all archive rulesets will attempt to run.
